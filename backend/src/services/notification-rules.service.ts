...
const EVENT_PLACEHOLDER_CATALOG: Record<string, string[]> = {
  ...
  plan_finish_by_wake: ['event', 'timestamp', 'timestamp_time', 'timestamp_date', 'planId', 'planName', 'targetSoc', 'finishBy'],
  plan_finish_by_scheduled: ['event', 'timestamp', 'timestamp_time', 'timestamp_date', 'planId', 'planName', 'targetSoc', 'currentSoc', 'chargeStartsAt', 'chargeStartsAt_time', 'finishBy', 'estimatedChargeHours', 'safetyMarginPct'],
...
const EVENT_PAYLOAD_PRESETS: Record<string, Record<string, unknown>> = {
  ...
  plan_finish_by_wake: { planId: '1', planName: 'Ricarica Notturna', targetSoc: 90, finishBy: new Date(Date.now() + 8 * 3600_000).toISOString() },
  plan_finish_by_scheduled: {
    planId: '1',
    planName: 'Ricarica Notturna',
    targetSoc: 90,
    currentSoc: 30,
    chargeStartsAt: new Date(Date.now() + 4 * 3600_000).toISOString(),
    chargeStartsAt_time: '02:15',
    finishBy: new Date(Date.now() + 8 * 3600_000).toISOString(),
    estimatedChargeHours: 3.6,
    safetyMarginPct: 10,
  },
...
const EVENT_PAYLOAD_SCHEMAS: Record<string, NotificationEventSchema> = {
  ...
  plan_finish_by_wake: {
    required: ['planId', 'planName', 'targetSoc', 'finishBy'],
    fields: { planId: 'string', planName: 'string', targetSoc: 'number', finishBy: 'string' },
  },
  plan_finish_by_scheduled: {
    required: ['planId', 'planName', 'targetSoc', 'currentSoc', 'chargeStartsAt', 'chargeStartsAt_time', 'finishBy', 'estimatedChargeHours'],
    fields: {
      planId: 'string',
      planName: 'string',
      targetSoc: 'number',
...
const PLACEHOLDER_DESCRIPTIONS: Record<string, string> = {
  ...
  currentSoc: 'Livello carica attuale al momento del calcolo (%)',
  ...
  finishBy: 'Scadenza entro cui terminare la ricarica (ISO)',
  chargeStartsAt: 'Orario calcolato di avvio ricarica (ISO)',
  chargeStartsAt_time: 'Orario calcolato di avvio ricarica (HH:MM)',
  estimatedChargeHours: 'Durata stimata della ricarica (ore)',
  safetyMarginPct: 'Margine di sicurezza applicato al calcolo (%)',
}
