...
- **OTA updater robustness**: before branch checkout, OTA now auto-stashes local changes (including untracked files) to avoid `checkout` aborts; dependency step uses `npm ci` with safe fallback to `npm install --no-package-lock`, preventing lockfile drift from breaking subsequent updates
