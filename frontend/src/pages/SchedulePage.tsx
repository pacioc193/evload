...
import { Calendar, Clock, Zap, Thermometer, Trash2, Plus } from 'lucide-react'
import {
  getScheduledCharges,
...
export default function SchedulePage() {
  ...
  const handleAddCharge = async () => {
    ...
    setChargeMsg('')
    try {
      await createScheduledCharge(new Date(chargeAt).toISOString(), chargeSoc, chargeAmps)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-evload-surface border border-evload-border rounded-xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
          <div className="sm:col-span-1">
            <label className="block text-sm text-evload-muted mb-1 flex items-center gap-1">
          ...
          <div>
            ...
            <input
              type="range" min={1} max={100} value={chargeSoc}
          ...
          <div>
            ...
            <input
              type="range" min={5} max={32} value={chargeAmps}
        ...
        {chargeMsg && (
          <p className={`text-sm ${chargeMsg.includes('Failed') ? 'text-evload-error' : 'text-evload-success'}`}>
            {chargeMsg}
          </p>
        )}
        ...
        {loadingCharges ? (
          ...
          <div className="space-y-2">
            {charges.map((sc) => (
              <div
                key={sc.id}
                className="flex items-center justify-between p-3 rounded-lg border border-evload-border bg-evload-bg"
              >
                ...
                <button
                  onClick={() => handleDeleteCharge(sc.id)}
      ...
      <div className="bg-evload-surface border border-evload-border rounded-xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
          ...
          <div>
            <label className="block text-sm text-evload-muted mb-1">
              Target Temperature: {climateTemp}°C
            </label>
            <input
              type="range" min={15} max={30} value={climateTemp}
        ...
        {climateMsg && (
          <p className={`text-sm ${climateMsg.includes('Failed') ? 'text-evload-error' : 'text-evload-success'}`}>
            {climateMsg}
          </p>
        )}
          ...
          <div className="space-y-2">
            {climates.map((sc) => (
              ...
              >
                ...
                <button
                  onClick={() => handleDeleteClimate(sc.id)}
                  className="p-1 text-evload-muted hover:text-evload-error transition-colors"
                >
                  <Trash2 size={16} />
                </button>
